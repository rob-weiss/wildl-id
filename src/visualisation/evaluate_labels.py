...
def main():
    ...
    threshold_pos_int = (
        ...
        if isinstance(threshold_position, (int, np.integer))
        else 0
    )
    ...
    for idx, species in enumerate(top_species):
        ...
        ax.set_title(
            f"{species.capitalize()} (n={len(species_data)})", fontweight="bold"
        )
        ax.set_xlabel("Hour")
        ax.set_ylabel("Count")
    ...
    plot_num += 1
    plt.savefig(
        output_dir / f"{plot_num:02d}_species_activity_patterns.svg",
        bbox_inches="tight",
    )
    ...
    plot_num += 1
    plt.savefig(
        output_dir / f"{plot_num:02d}_lighting_analysis.svg", bbox_inches="tight"
    )
    ...
    if len(other_species_cols) > 0:
        ...
        location_species_grouped["other"] = location_species_all[
            other_species_cols
        ].sum(axis=1)
    else:
        location_species_grouped = location_species_all[main_species_cols].copy()
    ...
    plot_num += 1
    plt.savefig(
        output_dir / f"{plot_num:02d}_location_comparison.svg", bbox_inches="tight"
    )
    ...
    if len(df_valid) > 0:
        ...
        if len(df_humans) > 0 and len(df_target_wildlife) > 0:
            ...
            for species_name, species_info in target_species_data.items():
                ...
                daily_species = (
                    df_species.groupby("date").size().reset_index(name="count")
                )
                ...
                daily_data = all_dates.merge(
                    daily_species, on="date", how="left"
                ).fillna(0)
                ...
                plt.savefig(
                    output_dir
                    / f"{plot_num:02d}_baiting_effect_{filename_species}.svg",
                    bbox_inches="tight",
                )
    ...
    if len(df_temp) > 0:
        ...
        for species in target_species:
            ...
            if len(species_data) >= 10:  # Only create plot if sufficient data
                ...
                ax1.set_title(
                    "Temperature vs Time of Day", fontsize=11, fontweight="bold"
                )
                ax1.grid(True, alpha=0.3)
                ...
                ax2.set_title(
                    "Temperature Distribution", fontsize=11, fontweight="bold"
                )
    ...
    if len(df_valid) > 0:
        # Get top species (excluding 'none')
        top_species = (
            species_counts[species_counts.index != "none"].head(8).index.tolist()
        )
    ...
    if len(df_valid) > 0:
        ...
        location = LocationInfo(
            "Camera Location", "Germany", TIMEZONE, LATITUDE, LONGITUDE
        )
        ...
        df_valid = df_valid[
            df_valid["sunset"].notna() & df_valid["sunrise"].notna()
        ].copy()
        ...
        if len(df_valid) > 0:
            ...
            if len(df_target) > 0:
                ...
                for species in [s for s in target_species if s == "roe deer"]:
                    ...
                    plt.setp(
                        ax_main.xaxis.get_majorticklabels(), rotation=45, ha="right"
                    )
            ...
            for species in [s for s in target_species if s == "roe deer"]:
                ...
                ax_dist.axhline(
                    y=0, color="gold", linestyle="--", linewidth=2, alpha=0.7
                )
            ...
            for species in target_species:
                ...
                cbar.set_label(
                    "Hours from Sunset", rotation=270, labelpad=20, fontsize=10
                )
    ...
    if len(df_valid) > 0:
        ...
        if len(df_solunar) > 0:
            ...
            for idx, species in enumerate(target_species):
                ...
                ax.set_xticklabels(
                    phase_counts.index, rotation=45, ha="right", fontsize=9
                )
                ax.set_ylabel("Number of Detections", fontsize=11)
                ax.set_title(
            ...
            plot_num += 1
            plt.savefig(
                output_dir / f"{plot_num:02d}_solunar_comparison.svg",
                bbox_inches="tight",
            )
            ...
            for species in target_species:
                ...
                if len(species_data) > 0:
                    ...
                    print(
                        f"  Activity during bright moon (75-100%): {full_moon_pct:.1f}%"
                    )
                    ...
                    print(
                        f"  Activity when moon above horizon: {above_horizon_pct:.1f}%"
                    )
        else:
